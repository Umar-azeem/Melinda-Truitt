...
export default function Footer() {
      ...
      <div className="flex flex-col gap-4 md:flex-row md:justify-between md:items-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <Link href="/contact-us">
            <button className="bg-[#E46A52] px-6 py-3 rounded-xl transition-transform duration-300 hover:translate-y-1 w-full sm:w-auto">
              Contact us
            </button>
          </Link>
        </div>
  ...
  },
  {
    src: icons.instagram,
    alt: "Instagram",
    href: "https://instagram.com/melindatruitt",
  },
  {
    src: icons.linkedin,
    alt: "LinkedIn",
  ...
  },
  {
    src: icons.x,
    alt: "X (Twitter)",
    href: "https://x.com/MelindaTruitt10", // replace with your X profile link
  }, ].map(({ src, alt, href }) => (
              <Link
                key={alt}
                href={href}
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 rounded-full text-white bg-white flex items-center justify-center flex-shrink-0 hover:bg-[#E46A52] transition-colors"
              >
                <Image
                  src={src}
                  alt={alt}
        ...
        <div className="flex flex-row-reverse md:flex-col items-center md:items-end justify-between md:justify-end gap-6 md:gap-0">
          <button
            onClick={scrollToTop}
            className="w-14 h-14 rounded-full bg-[#E46A52] flex items-center justify-center flex-shrink-0 transition-transform hover:-translate-y-1 active:translate-y-0"
